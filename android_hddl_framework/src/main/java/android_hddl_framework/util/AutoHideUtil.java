...
public class AutoHideUtil {
    ...
    public static void applyListViewAutoHide(Context context, ListView listView, View header, View footer,int scrollHeight,int headerHeight) {
        ListViewAutoHideTool tool = new ListViewAutoHideTool();
        tool.applyAutoHide(context, listView, header, footer, scrollHeight ,headerHeight);
    }
    /**
     * AutoHideUtil.applyListViewAutoHide()是一个帮助类，以快速实现MainActivity里面的滑动隐藏方法
    ...
    private static class ListViewAutoHideTool {
        ...
        public void applyAutoHide(Context context, ListView listView, View header, View footer,int scrollHeight,int headerHeight) {
            touchSlop = (int) (ViewConfiguration.get(context).getScaledTouchSlop() * 0.9);
            this.listView = listView;
            this.header = header;
            this.footer = footer;
        ...
        private void animateHide() {
            ...
            if (hideAnimatorSet != null && hideAnimatorSet.isRunning()) {
                ...
                ArrayList<Animator> animators = new ArrayList<Animator>();
                if (header != null) {
                    ObjectAnimator headerAnimator = ObjectAnimator.ofFloat(header, "translationY", header.getTranslationY(), scrollHeight!=0?-scrollHeight:-header.getHeight());
                    animators.add(headerAnimator);
                }
                if (footer != null) {
